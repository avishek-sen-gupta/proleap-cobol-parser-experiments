...
public class CobolParserRunnerImpl implements CobolParserRunner {
	...
	protected void analyze(final Program program) {
        System.out.println("NOW ANALYSZING");
	...
	public CobolParserParams createDefaultParams(final CobolSourceFormatEnum format, final File cobolFile) {
	...
	protected void parsePreprocessInput(final String preProcessedInput, final String compilationUnitName,
		...
		if (!params.getIgnoreSyntaxErrors()) {
			...
			lexer.addErrorListener(new ThrowingErrorListener(preProcessedInput));
		}
		...
		if (!params.getIgnoreSyntaxErrors()) {
			...
			parser.addErrorListener(new ThrowingErrorListener(preProcessedInput));
		}
